...
app.get('/test',async(req,res)=>{
  ...
  try {     
            ...
            const [[getStaffNameMissingReport]] = await pool.execute(`CALL GetLastWeekMissingTimesheetReport(${id})`);
            ...
            if (getStaffNameMissingReport && getStaffNameMissingReport.length > 0) {
                ...
                getStaffNameMissingReport.forEach(val => {
                    if (!processedStaff.has(val?.staff_id)) {
                       ...
                       console.log("val?.staff_fullname ",val?.staff_fullname);
            ...
            `;
                ...
                const [[roleRow]] = await pool.execute('SELECT r.role FROM staffs s JOIN roles r ON s.role_id = r.id WHERE s.id = ?', [id]);
                ...
                if (['SUPERADMIN', 'ADMIN', 'MANAGEMENT'].includes(roleName)) {
                    ...
                    relevantStaffIds = await getAllSubordinates(pool, id);
            ...
            `;
                ...
                let toEmail =  'Amritanshu.Pranjaya@outbooks.com';
                let subjectEmail = `Missing Timesheet Report - ${lastWeekStr}`;
                const filename = `MissingTimesheetReport_${shortLastWeekStr}.xlsx`;

                let htmlEmail = `<h3>Please find the attached Missing Timesheet Report for ${lastWeekStr}.</h3>`;
                ...
                console.log(`ℹ️ No missing timesheet report for`);
            }
        } catch (err) {
            console.error(`❌ Failed for: ${err.message}`);
        }

 return res.send("0000000000000000000")
});
