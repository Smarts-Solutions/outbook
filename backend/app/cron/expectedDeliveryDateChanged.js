  ...
  try {
    ...
    if (result && result.length > 0) {
      result?.forEach((val) => {
        jobDataList.push({
          ...
          "Expected Delivery Date ": val.expected_delivery_date || " - ",
          "Expected Delivery Date Old": val.expected_delivery_date_old || " - ",
    ...
    await processWithLimit(uniqueUsers, 5, async (user) => {
      try {
        ...
        summaryData.push({ "Summary": "Total Jobs with Changed Expected Delivery Dates", "Total Count": finalJobData.length });
        ...
        const filename = `Jobs where Expected delivery date has changed. This field had some data before and now has been changed - ${new Date().toISOString().slice(0, 10)}.xlsx`;
