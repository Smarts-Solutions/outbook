  ...
  if (result && result.length > 0) {
    ...
    await processWithLimit(uniqueUsers, 5, async (user) => {
      try {
        ...
        xlsx.utils.book_append_sheet(wb, xlsx.utils.json_to_sheet(finalJobData), "Sitting Over A Month");

        const excelBuffer = xlsx.write(wb, { type: 'buffer', bookType: 'xlsx' });

        const filename = `Jobs that have been sitting with us for over a month - ${new Date().toISOString().slice(0, 10)}.xlsx`;
