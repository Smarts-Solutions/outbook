...
parentPort.on("message", async (rows) => {
  ...
  if (result && result.length > 0) {
    ...
    await processWithLimit(uniqueUsers, 500, async (user) => {
      try {
        ...
        summaryData.push({ "Summary": "Total Jobs with Missing Paperwork Due in Next 2 Days", "Total Count": finalJobData.length });
        ...
        const excelBuffer = xlsx.write(wb, { type: 'buffer', bookType: 'xlsx' });
         ...
         const sent = await commonEmail(toEmail, subjectEmail, htmlEmail, "", "", excelBuffer, filename);
          ...
          logEmail({
            ...
            logFileName: "missingPaperworkInMax2Days.json",
          });
