  ...
  for (const row of rows) {
    try {
      ...
      if (getStaffNameMissingReport && getStaffNameMissingReport.length > 0) {
        ...
        xlsx.utils.book_append_sheet(wb, xlsx.utils.json_to_sheet(tab2Data), "Missing Managers");
        xlsx.utils.book_append_sheet(wb, xlsx.utils.json_to_sheet(tab3Data), "Grouped By Manager");
        xlsx.utils.book_append_sheet(wb, xlsx.utils.json_to_sheet(tab4Data.length ? tab4Data : [{"Message": "No consistent defaulters found."}]), "Consistent Defaulters");
        xlsx.utils.book_append_sheet(wb, xlsx.utils.json_to_sheet(tab5Data.length ? tab5Data : [{"Message": "No improvements found."}]), "Improved Last Week");
        ...
        let managerBreakdownHtml = "<ul>";
        for (const [managerId, data] of Object.entries(groupedByManager)) {
            managerBreakdownHtml += `<li><b>${data.manager_name}</b>: ${data.employees.length} employees</li>`;
        }
        managerBreakdownHtml += "</ul>";

        let htmlEmail = `
            <h3>Weekly Missing Timesheet Summary</h3>
            ...
            <ul>
                ...
                <li>Total managers who did not submit timesheet (Last Week): <b>${totalMissingManagers}</b></li>
                <li>Employees consistently missing for 4 weeks: <b>${totalDefaulters}</b></li>
                <li>Employees showing improvement (Submitted this week after missing previously): <b>${totalImproved}</b></li>
            </ul>
            <p><b>Missing Timesheets by Manager:</b></p>
            ${managerBreakdownHtml}
        `;
