...
const createCountry = async (Country) => {
    ...
    const checkQuery = `SELECT 1 FROM countries WHERE name = ? OR code = ?`;
    const query = `
    INSERT INTO countries (name, code, currency)
    VALUES (?, ?, ?)
    `;
    try {
        const [check] = await pool.query(checkQuery, [name, code]);

        if (check.length > 0) {
            return { status: false, message: 'Country name or code already exists.' };
        }
...
const updateCountry = async (Country) => {
    ...
    WHERE id = ?
    `;
    const checkQuery = `SELECT 1 FROM countries WHERE (name = ? OR code = ?) AND id != ?`;
    try {
        const [check] = await pool.query(checkQuery, [name, code, id]);
        if (check.length > 0) {
            return { status: false, message: 'Country name or code already exists.' };
        }
