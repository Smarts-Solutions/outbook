  ...
  if (customer_id == undefined || customer_id == null || customer_id == "") {
    ...
    if (customer.CustomerType == "1") {
      ...
      if (check.length > 0) {
        return {
          status: false,
          message: "Customer Trading Name Already Exists.",
        };
      }
    ...
    `;
      ...
      try {
        ...
        `;
        let phone_code =
          customer.phone_code == undefined ? "" : customer.phone_code;
    ...
    `;
      ...
      try {
        ...
        for (const detail of contactDetails) {
          ...
          let phone = detail.phoneNumber;
          let email = detail.email;
          let authorised_signatory_status =
            detail.authorised_signatory_status == true ? "1" : "0";

          const [result3] = await pool.execute(query2, [
            customer_id,
            role,
            first_name,
            last_name,
    ...
    if (check.length > 0) {
      return {
        status: false,
        message: "Customers Trading Name Already Exists.",
      };
    }
    ...
    // Company Details
      ...
      try {
        ...
        const [existIdResult] = await pool.execute(
          "SELECT id FROM customer_contact_details WHERE customer_id = ?",
          [customer_id]
        );
        ...
        for (const detail of contactDetails) {
          ...
          if (
            ...
            const query4 = `
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    `;
            const [result3] = await pool.execute(query4, [
              customer_id,
...
const getCustomer = async (customer) => {
  ...
  const [rows] = await pool.execute(
    'SELECT id , role_id  FROM staffs WHERE id = "' + staff_id + '" LIMIT 1'
  );
...
const updateProcessCustomerServices = async (customerProcessData) => {
  ...
  const [ExistCustomer] = await pool.execute(
    "SELECT customer_type , customer_code , account_manager_id  FROM `customers` WHERE id =" +
      customer_id
  );
      ...
      if (account_manager_id.length > 0) {
        for (const ac_id of account_manager_id) {
          ...
          if (existing2.length === 0) {
            await pool.execute(insertManagerQuery, [
              customer_service_id,
              ac_id,
            ]);
          }
        ...
        const [rows] = await pool.execute(selectManagerQuery, [
          customer_service_id,
          account_manager_id_exit,
        ]);
        ...
        if (!exists) {
          const insertManagerQuery1 = `
                ...
                VALUES (?, ?)
                `;
          await pool.execute(insertManagerQuery1, [
            customer_service_id,
            account_manager_id_exit,
          ]);
        }
        ...
        if (!exists) {
          const insertManagerQuery2 = `
                ...
                VALUES (?, ?)
                `;
          await pool.execute(insertManagerQuery2, [
            customer_service_id,
            account_manager_id_exit,
          ]);
        }
        ...
        if (customer_service_task.length > 0) {
          for (const cst of customer_service_task) {
            const checkQuery3 = `
                   ...
                   `;
            const [existing3] = await pool.execute(checkQuery3, [
              customer_id,
              service_id,
              cst,
            ]);
            if (existing3.length === 0) {
              await pool.execute(insertCSTQuery, [
                customer_id,
                service_id,
                cst,
              ]);
  ...
  if (Task.length > 0) {
    ...
    `;
    const [existingChecklist] = await pool.execute(checkQueryChecklist, [
      customer_id,
      serviceId,
      JobTypeId,
      client_type_id,
    ...
    if (existingChecklist.length === 0) {
      const insertChecklistQuery = `
        ...
        VALUES (?, ?, ?, ?, ?)
        `;
      const [result] = await pool.execute(insertChecklistQuery, [
        customer_id,
        serviceId,
        JobTypeId,
        client_type_id,
  ...
  let pageStatus = "2";
  await pool.execute("UPDATE customers SET form_process = ? WHERE id = ?", [
    pageStatus,
    customer_id,
  ]);
...
const updateProcessCustomerEngagementModel = async (customerProcessData) => {
  ...
  const [updateQuery] = await pool.execute(customerUpdateQuery, [
    customerJoiningDate,
    customerSource,
    customerSubSource,
    customer_id,
  ...
  if (fte_dedicated_staffing === "1") {
    ...
    const [exist1] = await pool.execute(checkQuery1, [
    ...
    if (exist1.length === 0) {
      ...
      await pool.execute(updateQueryEngagementModel, [
        fte_dedicated_staffing,
        customer_id,
      ]);
    } else {
  ...
  if (percentage_model === "1") {
    ...
    const [exist2] = await pool.execute(checkQuery2, [
      customer_engagement_model_id,
    ]);
  ...
  if (adhoc_payg_hourly === "1") {
    ...
    const [exist3] = await pool.execute(checkQuery3, [
      customer_engagement_model_id,
    ]);
  ...
  if (customised_pricing === "1") {
    const { customer_id, customised_pricing, customised_pricing_data } =
      customerProcessData;
    ...
    if (customised_pricing_data.length > 0) {
      for (const customisedVal of customised_pricing_data) {
        ...
        const [exist4] = await pool.execute(checkQuery4, [
          customer_engagement_model_id,
  ...
  // Delete Entry
  if (
    fte_dedicated_staffing === "0" ||
    percentage_model === "0" ||
    adhoc_payg_hourly === "0" ||
    customised_pricing === "0"
  ) {
...
const updateProcessCustomerFile = async (
  customerProcessDataFiles,
  customer_id
) => {
...
const getSingleCustomer = async (customer) => {
  ...
  const [ExistCustomer] = await pool.execute(
    "SELECT customer_type FROM `customers` WHERE id =" + customer_id
  );
    ...
    if (rows.length > 0) {
      ...
      const services = rows.map((row) => ({
        ...
        account_manager_ids: row.account_manager_ids
          ? row.account_manager_ids.split(",").map(Number)
          : [],
      }));
...
const customerUpdate = async (customer) => {
  ...
  if (pageStatus === "1") {
        ...
        for (const detail of contactDetails) {
          ...
          if (
            ...
            const query4 = `
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                        `;
            result4 = await pool.execute(query4, [
              customer_id,
              customer_contact_person_role_id,
              first_name,
              last_name,
            ...
            arrayInterId.push(contact_id);
            result3 = await pool.execute(query3, [
              customer_contact_person_role_id,
              first_name,
              last_name,
              phone_code,
    ...
    // Partnership Details
      ...
      try {
        ...
        for (const detail of contactDetails) {
          ...
          if (
            ...
            const query4 = `
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        `;
            const [result3, err3] = await pool.execute(query4, [
              customer_id,
              customer_contact_person_role_id,
              first_name,
              last_name,
        ...
        if (
          logAdditional == true &&
          (result.changedRows > 0 || logUpdateRequired == true)
        ) {
          const currentDate = new Date();
          await SatffLogUpdateOperation({
            staff_id: customer.StaffUserId,
  ...
  //  Page Status 2 Service Part
    ...
    const [ExistServiceids] = await pool.execute(
      "SELECT service_id  FROM `customer_services` WHERE customer_id =" +
        customer_id
    );
    const [ExistCustomer] = await pool.execute(
      "SELECT customer_type , customer_code , account_manager_id  FROM `customers` WHERE id =" +
    ...
    for (const serVal of services) {
      ...
      try {
        ...
        // Process 2 table
        const [[exist_count_manager]] = await pool.execute(
          "SELECT COUNT(*) as exist_count_manager FROM `customer_service_account_managers`WHERE customer_service_id = ?",
          [customer_service_id]
        );

        await pool.execute(
          "DELETE FROM customer_service_account_managers WHERE customer_service_id = ?",
          [customer_service_id]
        );
        ...
        const [[new_count_manager]] = await pool.execute(
          "SELECT COUNT(*) as new_count_manager FROM `customer_service_account_managers`WHERE customer_service_id = ?",
          [customer_service_id]
        );

        if (
          exist_count_manager.exist_count_manager !==
          new_count_manager.new_count_manager
        ) {
          logUpdateRequired = true;
        }
      } catch (err) {
    ...
    if (deleteIdArray.length > 0) {
      ...
      for (const id of deleteIdArray) {
        const query = `
           ...
           `;
        const [ExistCustomeServicesId] = await pool.execute(query, [
          customer_id,
          id,
        ]);
    ...
    return {
      status: true,
      message: "customer services update successfully.",
      data: customer_id,
    };
  }
  ...
  //  Page Status 3 Customer engagement model Part
    ...
    if (customised_pricing === "1") {
      ...
      const [existPricingData] = await pool.execute(
        "SELECT id FROM customer_engagement_customised_pricing WHERE customer_engagement_model_id = ?",
        [customer_engagement_model_id]
      );
      ...
      if (customised_pricing_data.length > 0) {
        for (const customisedVal of customised_pricing_data) {
          ...
          if (
            ...
            customised_pricing_id == undefined ||
            customised_pricing_id == null
          ) {
            const insertQuery = `
                            INSERT INTO customer_engagement_customised_pricing (
                                customer_engagement_model_id,
      ...
      const updateQueryEngagementModel = `UPDATE customer_engagement_model SET customised_pricing = ? WHERE customer_id = ? `;
      const [rows] = await pool.execute(updateQueryEngagementModel, [
        customised_pricing,
        customer_id,
      ]);
    ...
    if (
      ...
      if (fte_dedicated_staffing === "0") {
        ...
        const updateQueryEngagementModel = `UPDATE customer_engagement_model SET fte_dedicated_staffing = ? WHERE customer_id = ? `;
        const [result] = await pool.execute(updateQueryEngagementModel, [
          fte_dedicated_staffing,
          customer_id,
        ]);
        if (result.changedRows > 0) {
    ...
    return {
      status: true,
      message: "customers model updated successfully.",
      data: customer_id,
    };
  }
