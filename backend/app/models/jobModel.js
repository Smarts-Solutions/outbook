...
const getAddJobData1 = async (job) => {
  ...
  try {
    ...
    // Compute this once — was being re-derived 3 separate times before (2x in query building,
    // 1x in the formatting section below). Same checks, just no repeated work / repeated typos risk.
    const isSuperAdmin = roleData?.length > 0 && roleData[0]?.role_name === "SUPERADMIN";
    ...
    let queryCustomerWithServices = `
      ...
      ORDER BY services.id DESC;
    `;
    if (roleData?.length > 0 && !isSuperAdmin) {
      queryCustomerWithServices = roleData[0]?.role_id == 4 ? `
      SELECT services.id AS service_id, services.name AS service_name
      FROM customers
      JOIN customer_services ON customers.id = customer_services.customer_id
    ...
    // isSuperAdmin ke case me `client` hamesha `rows` se banta hai (formatting section me neeche dekho),
    // clientDataQuery ka result us case me kabhi use hi nahi hota — to superadmin ke liye ye heavy
    // JOIN+GROUP BY query fire hi mat karo. Baaki cases (non-superadmin) me pehle jaisa parallel hi
    // rakha hai kyunki RoleAccess ka result aane se pehle pata nahi chalta ki clientData chahiye ya nahi,
    // aur use sequentially wait karne se latency badh jaati — data flow same rehta hai, sirf ek
    // redundant round-trip superadmin ke case me bach jaata hai.
    const p_clientData = isSuperAdmin ? Promise.resolve([[]]) : pool.execute(clientDataQuery, [customer_id]);
    const p_rows7 = pool.execute(queryCustomerWithServices, [customer_id]);
    const p_roleAccess = (rows.length > 0 && roleData.length > 0 && !isSuperAdmin)
      ? pool.execute("SELECT * FROM role_permissions WHERE role_id = ? AND permission_id = ?", [roleData[0].role_id, 34])
      : Promise.resolve([[]]);
    const p_rows9 = queryAccountManeger ? pool.execute(queryAccountManeger) : Promise.resolve([[]]);
    ...
    if (rows.length > 0) {
      ...
      if (isSuperAdmin || RoleAccess.length > 0) {
        client = rows.map((row) => ({
          client_id: row.client_id,
          client_trading_name: row.client_trading_name,
          client_client_type: row.client_client_type,
    ...
    // job_type dedup: pehle O(n^2) tha (findIndex har element ke liye poora array scan karta tha).
    // Same result, ab O(n) — ek Map se seen ids track karte hain.
    let job_type = [];
    if (rows3.length > 0) {
      const seenJobTypeIds = new Map();
      for (const row of rows3) {
        if (!seenJobTypeIds.has(row.job_type_id)) {
          seenJobTypeIds.set(row.job_type_id, row);
        }
      }
      job_type = Array.from(seenJobTypeIds.values());
    }
    ...
    if (result.insertId > 0) {
      ...
      if (selectedStaffData && selectedStaffData.length > 0) {
        for (const staff of selectedStaffData) {
          let { value } = staff;
          const [[isExistJobId]] = await pool.execute(
            `SELECT id FROM job_allowed_staffs WHERE staff_id = ${value} AND job_id = ${result.insertId}`
          );
          if (!isExistJobId) {
            const query = `
...
const getJobByCustomer = async (job) => {
  ...
  try {
    // If no customer_id, use sidebar function
    if (!customer_id) {
    ...
    // SUPERADMIN or permission access
    if (
      rows.length > 0 &&
  ...
  try {
    ...
    if (
      ...
      const query = `
        ...
        LEFT JOIN master_status ON master_status.id = jobs.status_type
        LEFT JOIN timesheet ON timesheet.job_id = jobs.id AND timesheet.task_type = '2'
        WHERE jobs.customer_id IN (${customer_id})
        GROUP BY jobs.id
        ORDER BY jobs.id DESC
      `;
      [result] = await pool.execute(query);
  ...
  try {
    ...
    if (
      ...
      const query = `
        ...
        timesheet ON timesheet.job_id = jobs.id AND timesheet.task_type = '2'
        WHERE
        jobs.client_id IN (${client_id}) OR jobs.customer_id IN (${customer_id})
        GROUP BY jobs.id
        ORDER BY 
         jobs.id DESC;
        `;
  ...
  try {
    ...
    SELECT 
     ...
     jobs.Management_Accounts_Frequency_id_6 AS Management_Accounts_Frequency_id_6,
      ...
      jobs.processing_checklist AS processing_checklist,
      jobs.reviewing_checklist AS reviewing_checklist,
      jobs.processing_checklist_status AS processing_checklist_status,
      jobs.reviewing_checklist_status AS reviewing_checklist_status,
      jobs.checklist_modal_data AS checklist_modal_data,
   ...
   WHERE job_id = ?`,
    ...
    if (rows.length > 0) {
      ...
      result = {
        ...
        Management_Accounts_Frequency_id_6:
          rows[0].Management_Accounts_Frequency_id_6,
        ...
        Management_Accounts_FromDate_id_6:
          rows[0].Management_Accounts_FromDate_id_6,
        Management_Accounts_ToDate_id_6:
          rows[0].Management_Accounts_ToDate_id_6,
 ...
 `;
  try {
    ...
    if (expected_delivery_date == null) {
      ...
      ExistJob.expected_delivery_date_old == null
    ) {
...
const updateJobStatus = async (job) => {
  ...
  try {
    ...
    if ([4, 5, 7, 3].includes(parseInt(status_type))) {
      ...
      if (
        ["", null, undefined, 0, "0"].includes(ExistAllocatedTo?.allocated_to)
      ) {
        return {
          status: false,
...
const GetJobStatus = async (job) => {
  ...
  try {
    ...
    if (result.length > 0) {
      ...
      return {
        status: false,
        message: "No job found with the given status_id.",
      };
    }
  } catch {
    console.log("DDD", err);
