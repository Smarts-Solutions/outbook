...
const createMasterStatus = async(masterStatus) => {
    ...
    try {
        ...
        return {status: true, message: 'Master Status created successfully.' , data : result.insertId};
    } catch (err) {
        console.error('Error inserting data:', err);
        throw err;
    }
...
const updateMasterStatus = async(masterStatus)=>{
    ...
    try {
        ...
        const [result] = await pool.execute(query, values);
        return { status: true, message: 'Master Status updated successfully.', data: result.affectedRows };
    } catch (err) {
        console.error('Error updating data:', err);
        throw err;
    }
