  ...
  const roleData = async () => {
    try {
      const response = await dispatch(
        Role({ req: { action: "staffRole" }, authToken: token }),
      ).unwrap();
      if (response.status) {
      ...
      setCheckboxState((prevState) => {
        ...
        if (permission_name === "all_clients" && !checked) {
          updatedState = updatedState.map((item) =>
            item.permission_name === "all_jobs"
              ? { ...item, is_assigned: false }
              : item,
          );
        } else if (permission_name === "all_jobs" && checked) {
          const allClientsExist = updatedState.some(
            (item) => item.permission_name === "all_clients",
          );
          if (allClientsExist) {
            updatedState = updatedState.map((item) =>
              item.permission_name === "all_clients" ||
              item.permission_name === "all_customers"
            ...
            if (data.is_assigned === 1) {
              setCheckboxState((prevState) => [
                ...prevState,
                {
                  ...
                  role_id: val.id,
                  is_assigned: data.is_assigned === 1,
                ...
                <div className="accordion-item mt-2" key={index}>
                  ...
                  >
                    <div className="accordion-body">
                      <div className="row">
                        {accessData &&
                          accessData.data.map((section, index) => (
                            <div key={index} className="col-lg-2 col-md-6">
                              <AccordionItem
                                section={section}
                                TradingName={val.role_name}
                                role_id={val.id}
                              />
                            </div>
                          ))}
