...
const DiscrepancyReportProcessor = () => {
  ...
  const Discrepancy = async () => {
    ...
    await dispatch(getAllTaskByStaff(data))
      ...
      .then((res) => {
        if (res.status) {
          console.log("Discrepancy discrepancyReportProcessor:", res);
  ...
  const columns = [
   {
...

    {
      ...
      selector: row => convertTimeFormatString(row.job_total_time_processor),
      sortable: true
    },
...
export default DiscrepancyReportProcessor