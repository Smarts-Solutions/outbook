...
import { JobType, GetServicesByCustomers, GETTASKDATA, getList, UpdateChecklistData } from '../../../../ReduxStore/Slice/Settings/settingSlice';
...
const CreateCheckList = () => {
    ...
    const [tasks, setTasks] = useState([{ task_id: "", task_name: '', budgeted_hour: '', checklist_tasks_id: '' }]);
    ...
    useEffect(() => {
        if (location.state?.checklist_id) {
            let req = { action: "getById", checklist_id: location.state.checklist_id };
            ...
            dispatch(getList(data))
                ...
                .then(response => {
                    if (response.status) {
                        ...
                        if (checklistData.task) {
                            setTasks(checklistData.task.map(task => ({
                ...
                .catch(error => console.log("Error fetching checklist data:", error));
        }
                ...
                .then(response => {
                    if (response.status) {
                        ...
                        setFormData(prevData => ({
                            ...prevData,
                            service_id: response.data.find(service => service.service_id === prevData.service_id) ? prevData.service_id : ''
                        }));
                    }
                })
        ...
        dispatch(getList(dataClientType))
            ...
            .then(response => {
                if (response.status) {
                    ...
                    setFormData(prevData => ({
                        ...prevData,
                        client_type_id: response.data.find(client => client.id === prevData.client_type_id) ? prevData.client_type_id : ''
                    }));
                }
            })
            .catch(error => console.log("Error fetching client types:", error));
    ...
    const addTask = () => {
        setTasks([...tasks, { task_name: '', budgeted_hour: '', task_id: null, checklist_tasks_id: null }]);
    };
    ...
    const handleSubmit = async () => {
        ...
        if (!formData.service_id) validationErrors.service_id = "Service Type is required";
        if (!formData.job_type_id) validationErrors.job_type_id = "Job Type is required";
        if (!formData.client_type_id) validationErrors.client_type_id = "Client Type is required";
        if (!formData.check_list_name) validationErrors.check_list_name = "Check List Name is required";
                            ...
                            <div className="row">
                                <div className="col-lg-12">
                                    ...
                                    >
                                        ...
                                        {jobTypeList && jobTypeList.map(job => (
                        ...
                        <div key={index} className="row mt-4">
                            ...
                            <div className="col-lg-4 d-flex align-items-center">
                                ...
                                {index === tasks.length - 1 && (
                                    <button type="button" className="btn btn-info ms-2" onClick={addTask}>Add Task</button>
                                )}
                            </div>
                        </div>
                    ))}
