...
import { GetAllCompany, GetOfficerDetails, CustomerClientAdd } from "../../../ReduxStore/Slice/Customer/CustomerSlice";
...
const CreateClient = () => {
  ...
  const AddClientFun = async (req) => {
    ...
    const data = { req: req, authToken: token };
    await dispatch(CustomerClientAdd(data))
      .unwrap()
      .then((response) => {
        if (response.status) {
          Swal.fire({
  ...
  const handleSubmit = async () => {
    ...
    if (selectClientType == 1 && validateAllFields(1)) {
      ...
      await dispatch(CustomerClientAdd({ req: req, authToken: token }))
        .unwrap()
        .then((response) => {
          if (response.status) {
            Swal.fire({
