...
const CustomerMissingLogs = ({ getAccessDataJob, goto }) => {
  ...
  const [showEditmissinglogsModal, setShowEditMissinglogsModal] =
    useState(false);
    ...
    if (getEditData && showEditmissinglogsModal) {
      setMissionAllInputLogData({
        ...
        missing_log_reviewed_by:
          getProfileDetails?.first_name + " " + getProfileDetails?.last_name,
  ...
  const handleChange = (e) => {
    ...
    if (name === "missing_log_document") {
      ...
      if (files && typeof files[Symbol.iterator] === "function") {
        ...
        setMissionAllInputLogData({
          ...missionLogAllInputData,
          missing_log_document: fileArray,
        });
      }
    } else {
      ...
      .then(async (response) => {
        if (response.status) {
          if (missionLogAllInputData.missing_log_document != null) {
            ...
            await dispatch(CustomerMissingLogAction({req: {
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (
      missionLogAllInputData.missing_log_reviewed_date == null ||
      missionLogAllInputData.missing_log_reviewed_date == ""
    ) {
      Swal.fire({
        icon: "warning",
        title: "Warning...",
    ...
    await dispatch(CustomerMissingLogAction(data))
      ...
      .then(async (response) => {
        if (response.status) {
          if (
            missionLogAllInputData.missing_log_document != null &&
            missionLogAllInputData.missing_log_document != undefined
          ) {
            setIsLoading(true);
            const invalidValues = [undefined, null, "", 0, "0"];
            let job_name = "JOB_DEMO";
  ...
  const columns = [
    ...
    {
      name: "Missing Log Sent On",
      selector: (row) => convertDate(row.missing_log_sent_on),
      reorder: false,
      sortable: true,
    },
    {
      name: "Missing Log Prepared Date",
      selector: (row) => convertDate(row.missing_log_prepared_date),
      reorder: false,
      sortable: true,
    },
    {
      name: "Missing Log Reviewed Date",
      selector: (row) => convertDate(row.missing_log_reviewed_date),
      reorder: false,
      sortable: true,
    },
    {
    ...
    {
      name: "Status",
      selector: (row) => (row.status == 1 ? "Completed" : "Incomplete"),
      reorder: false,
      sortable: true,
    },
  ...
  const handleExport = async () => {
    ...
    const response = await dispatch(CustomerMissingLogAction(data)).unwrap();
    ...
    const exportData = response?.data?.rows?.map((item) => ({
      "Missing Log Title": item.title || "-",
      "Missing Log Sent On": item.missing_log_sent_on || "-",
      "Missing Log Prepared Date": item.missing_log_prepared_date || "-",
      "Missing Log Reviewed Date": item.missing_log_reviewed_date || "-",
      "Last Chaser": item.last_chaser || "-",
      Status: item.status == 0 ? "Incomplete" : "Complete" || "-",
    }));

    downloadCSV(exportData, "Missing Logs Details.csv");
  };
    ...
    <div className={isLoading ? "blur-container" : ""}>
      ...
      <div className="">
        ...
        <div className="row align-items-center">
          {/* LEFT */}
          <div className="tab-title col-md-6">
            <h3 className="mb-0">Missing Logs</h3>
          </div>
            ...
            <div className="col-lg-6">
              <div className="mb-3">
                ...
                <select
                  ...
                  className={
                    errors1["missing_log"]
                      ? "error-field form-select mb-3 ismissinglog"
                      : "form-select mb-3 ismissinglog"
                  }
                  autoFocus
                ...
                {errors1["missing_log"] && (
                  <div className="error-text">{errors1["missing_log"]}</div>
                )}
              </div>
            </div>
            <div className="col-lg-6">
                ...
                <input
                  ...
                  className={
                    errors1["missing_log_document"]
                      ? "error-field custom-file-input form-control"
            ...
            <div className="row">
              ...
              <div className="col-md-6">
                <span className="text-muted">
                  {singleMissionData && singleMissionData?.missing_log_sent_on}
                </span>
              </div>
            </div>
            {/* <div className="row">
...
export default CustomerMissingLogs;
