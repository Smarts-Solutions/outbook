...
const CustomerQueries = ({ job_id, getAccessDataJob, goto }) => {
      ...
      .then(async (response) => {
        if (response.status) {
          if (AllQueryInputdata.QueryDocument != null) {
            ...
            await dispatch(CustomerQueryAction({req: {
  ...
  const handleExport = async () => {
    ...
    const response = await dispatch(CustomerQueryAction(data)).unwrap();
    ...
    const exportData = response?.data?.rows?.map((item) => ({
      ...
      "Query Sent Date": item.query_sent_date || "-",
      "Missing Queries Prepared Date":
        item.missing_queries_prepared_date || "-",
      "Final Query Response Received Date":
        item.final_query_response_received_date || "-",
      "Last Chaser": item.last_chaser || "-",
      Status: item.status == 1 ? "Complete" : "Incomplete" || "-",
    }));

    downloadCSV(exportData, "Queries Details.csv");
  };
  ...
  <button
        ...
        <div className="row align-items-center">
          ...
          <div className="tab-title col-md-8">
            <h3 className="mb-0">Queries</h3>
          </div>

          {/* RIGHT */}
          <div className="col-md-4 text-end">
            {(hasAccess("queries", "export") || role === "SUPERADMIN") && AllQueryList && AllQueryList.length > 0 && (
              <button
