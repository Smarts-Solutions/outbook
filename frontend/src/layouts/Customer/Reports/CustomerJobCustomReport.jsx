...
import { TextSelect, Download, Trash } from "lucide-react";
import { convertDate, convertDate1 } from "../../../Utils/Comman_function";
import { useNavigate } from "react-router-dom";
import ReactPaginate from "react-paginate";

function CustomerJobCustomReport() {
  ...
  const staffData = async (role_id, type) => {
    ...
    if (Number(role_id) == 4) {
      if (role?.toUpperCase() === "SUPERADMIN" || role?.toUpperCase() === "CUSTOMER") {
        var req = {
  ...
  const exportToCSV = async (data1) => {
    ...
    const data = { req: req, authToken: token };
    await dispatch(CustomerTimesheetReports(data))
      .unwrap()
      .then(async (response) => {
        setLoading(false);
  ...
  const GetAllJobs = async ({
    ...
    try {
      const response = await dispatch(CustomerGetAllTaskByStaff(data)).unwrap();
      if (response.status) {
...
export default CustomerJobCustomReport;
